// Standalone QRCode Generator (Pure JS, no dependencies, supports SVG, Canvas, DataURL)
(function(root, factory) {
    ...
    var QR = {
        ...
        generateSVG: function(text, options) {
            ...
            var qr = new QRCodeModel(options.typeNumber || 0, options.correctLevel !== undefined ? options.correctLevel : QR.CorrectLevel.M);
            
            // UTF-8 encoding support for Thai text & special characters
            ...
            for (var r = 0; r < count; r++) {
                for (var c = 0; c < count; c++) {
                    if (qr.isDark(r, c)) {
                        ...
                        svg += '<rect x="' + x.toFixed(2) + '" y="' + y.toFixed(2) + '" width="' + (cellSize + 0.1).toFixed(2) + '" height="' + (cellSize + 0.1).toFixed(2) + '" fill="' + colorDark + '"/>';
                    }
                }
            }
            svg += '</svg>';
        ...
        generateDataURL: function(text, options) {
            var svg = QR.generateSVG(text, options);
        ...
        drawToCanvas: function(canvas, text, options) {
            ...
            for (var r = 0; r < count; r++) {
                for (var c = 0; c < count; c++) {
                    if (qr.isDark(r, c)) {
                        ctx.fillRect(Math.round((c + margin) * cellSize), Math.round((r + margin) * cellSize), Math.ceil(cellSize), Math.ceil(cellSize));
                    }
                }
            }
        }
